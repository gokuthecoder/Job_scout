...
export default function All_placement() {
    ...
    const columns = useMemo(
        () => [
            {
                accessorFn: (row) => row.userName, //alternate way
                id: 'df,msdf', //id required if you use accessorFn instead of accessorKey
                header: 'User',
                Header: () => <i>User</i>, //optional custom header render
            },
            {
            ...
            },
            {
                ...
                (
                    ...
                    //     </Button>
                    // </Link>
                    <img src={BASE_IMG + row.original.image} alt={row.original.image} style={{ width: '100px' }} />
                ),
            },
            {
                accessorFn: (row) => row._id, //accessorFn used to join multiple data into a single cell
                ...
                (
                    ...
                    <Link to={'/admin/updateplacement/' + row.original._id}>
                        <Button variant="outlined" color="success">
                            Update
                        </Button>
                    </Link>
    ...
    return (
        <>
            <h1 className='text-center text-success my-5'>Placement</h1>
            <MaterialReactTable
                columns={columns}
                data={data}
